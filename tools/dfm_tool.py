...
DFM_ANALYSIS_SCHEMA = {
    "name": "dfm_analysis",
    "description": "Read bounded Check ontology context, plan, and manage non-blocking DFM runs. Use context only for the relevant Check instead of inferring DFM semantics from names. The external OCCT C++ analyzer is integrated as experimental; PythonOCC remains the reference STEP backend and NX/Parasolid remains an explicit optional backend. Unavailable analyzers fail explicitly; never infer engineering findings from that status.",
    "parameters": {"type": "object", "properties": {
        ...
        "analyzer_key": {"type": "string", "enum": ["occt", "step", "parasolid", "drawing", "fusion"]},
