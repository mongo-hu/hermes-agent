...
DFM_ANALYSIS_SCHEMA = {
    "name": "dfm_analysis",
    "description": "Run the DFM workflow. Drawing OCR is deterministic; use drawing_context and the current Hermes model to interpret explicit facts, then submit_observations for validated persistence. Use fusion_context and submit_fusion_links for Agent semantic proposals that the service checks against geometry IDs. The current PythonOCC STEP analyzer is a non-certified reference backend; unavailable analyzers fail explicitly.",
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
            ...
            "analyzer_key": {
                "type": "string",
                "enum": ["step", "parasolid", "drawing", "fusion"],
            },
            "idempotency_key": {"type": "string"},
        },
