...
规划以 **2026 年 9 月** 为时间基线。产品几何 DFM 采用半年左右一个能力台阶的建议节奏；
2028 年下半年以后的阶段只表示建议启动窗口，需在上一阶段验收后结合客户需求、数据条件、人员和
专项技术预研重新立项，不作为固定交付承诺。

## 2. DFM 产品能力全景 🧭

| 产品能力 | 主要解决的问题 | 核心技术 | 建议阶段与时间 |
| --- | --- | --- | --- |
| 🗂️ 项目与分析数据底座 | 一次分析如何复现，结论如何追溯 | Manifest、Snapshot、Stable ID、Artifact、版本与哈希 | V0.9｜2026.09—12 |
| 🧱 初级产品 DFM 分析 | 自动完成常规结构的基础几何检查、定位和报告 | OCCT C++、B-Rep 特征识别、区域化几何计算、默认规则 | V1.0｜2027 H1 |
| 🔎 中级产品 DFM 分析 | 结合二维信息、企业规则和多因素完成较完整评审 | 2D/3D Fusion、多 Measurement 规则、企业规则、置信度 | V1.1｜2027 H2 |
| 🧠 高级产品 DFM 分析 | 识别复杂关联风险，比较整改方案并复用历史经验 | 跨特征推理、案例检索、风险排序、反馈校准 | V1.2｜2028 H1 |
| 🧰 模具结构 DFM | 分型、侧向抽芯、顶出、浇注和冷却方案是否可行 | 模具结构语义、运动关系、空间可达性、方案评估 | V2.0｜2028 H2 起，专项立项 |
| 🌊 成型过程分析 | 充填、保压、冷却和翘曲风险如何评估 | 网格、材料流变数据、工艺边界、CAE Solver | V2.5｜2029 起，专项立项 |
| 🧠 联合方案优化 | 产品、模具和工艺方案如何综合选择 | 多方案自动运行、多目标优化、约束求解 | V3.0｜长期规划 |
| 🏭 企业质量闭环 | 分析结果如何与试模、量产和企业经验持续闭环 | PLM/MES/QMS 集成、数据治理、效果校准 | V4.0｜长期持续演进 |
...
以下 `Vx.x` 是产品能力版本；现有研发文档中的 `M2.5/M2.6/M3` 等属于内部技术里程碑，两套编号
分别用于产品汇报和研发管理。`H1/H2` 分别表示上半年和下半年；远期日期为启动窗口，不是固定
上线承诺。

```mermaid
flowchart LR
    V09["V0.9 · 2026.09—12<br/>技术与数据底座"] --> V10["V1.0 · 2027 H1<br/>初级产品 DFM"]
    V10 --> V11["V1.1 · 2027 H2<br/>中级产品 DFM"]
    V11 --> V12["V1.2 · 2028 H1<br/>高级产品 DFM"]
    V12 --> V20["V2.0 · 2028 H2 起<br/>模具结构 DFM"]
    V20 --> V25["V2.5 · 2029 起<br/>成型过程分析"]
    V25 --> V30["V3.0 · 长期规划<br/>联合方案优化"]
    V30 --> V40["V4.0 · 持续演进<br/>企业质量闭环"]
    ...
    class V09 base;
    class V10,V11,V12 geometry;
    class V20 mold;
    class V25,V30 simulation;
    class V40 enterprise;
```

### V0.9：技术与数据底座（2026.09—12）🧪
...
### V1.0：初级产品 DFM 分析（2027 H1）🧱

**阶段目标**

在限定产品类型和标准输入条件下，形成相当于初级 DFM 分析师的基础检查能力，把重复、客观、
规则明确的工作稳定自动化。

**核心交付**

- 独立 `dfm-occt-worker`，使用 OCCT C++ 完成生产级 STEP 处理；
- 识别主壁、螺钉柱、加强筋、凸台、孔、倒扣和外观面等常见结构区域；
- 计算壁厚、拔模角、圆角、孔深、距离和结构比例等基础指标；
- 使用经过审核的平台默认规则完成确定性判断，缺失的项目事实由用户确认；
- 输出问题等级、三维定位、证据图、规则依据和标准化 DFM 报告；
- 支持工程师逐条确认、驳回或修正分析结论。
...
在批准产品范围和 Golden Set 上，基础特征、指标和问题区域达到约定工程指标；系统可稳定批量运行，
初级检查结果可由模具工程师快速复核。

---

### V1.1：中级产品 DFM 分析（2027 H2）📐📚

**阶段目标**

从“单项基础检查”升级为较完整的项目评审：能够结合三维模型、二维图纸、用户事实、企业规则和
多个测量值处理条件变化与结构关联，能力目标接近中级 DFM 分析师。
...
- MySQL 本体库、规则库、默认规则、企业规则、审核、发布和版本回滚；
- 支持多条件因子、多 Measurement 表达式和结构间关系检查；
- AI 根据本体和知识引用起草规则，由工程师审核后发布；
- 按严重度、置信度和影响范围组织问题清单，减少逐项人工筛选。
...
二维信息能够稳定参与三维规则分析；企业规则更新无需发布 Agent 新版本；对常见复杂产品可形成覆盖
主要结构区域的完整评审，所有事实、测量和规则均可追溯。

---

### V1.2：高级产品 DFM 分析（2028 H1）🧠

**阶段目标**

从“按检查项发现问题”升级为“理解问题之间的联系并辅助方案权衡”，处理复杂产品和非标准结构，
能力目标接近高级 DFM 分析师在产品设计评审阶段的综合判断。

**核心交付**

- 处理复杂曲面、薄壁深腔、密集筋柱、局部过渡和多个相邻结构形成的耦合风险；
- 建立跨 Feature、Region、Metric 和 Check 的依赖关系，合并重复问题并识别根因候选；
- 结合历史案例、知识依据和企业经验生成多个可执行整改方案，说明收益、代价和约束；
- 对不确定识别、规则冲突和高风险结论主动请求工程师确认，不输出伪确定结论；
- 通过工程师复核、整改前后对比和实际项目反馈持续校准识别、排序与推荐效果；
- 支持项目级风险摘要、问题优先级和评审沟通材料自动生成。

**技术重难点 🔥**

- **跨特征关联分析**：缩痕、强度、脱模和外观风险可能由多个邻近结构共同造成；
- **复杂几何鲁棒性**：非标准建模、修复模型和高面数产品仍需保持区域身份与测量稳定；
- **经验与规则协同**：历史案例用于解释和方案参考，不能替代经过审核的阈值规则；
- **方案权衡**：减胶、加强、改圆角和调整拔模可能互相影响，需要明确约束和副作用；
- **置信度与人工边界**：系统必须知道何时可自动结论、何时只能给候选、何时必须转人工。

**阶段验收**

在复杂产品盲测和真实项目评审中，系统能够稳定发现关键关联风险、减少重复问题，并给出可解释、
可执行的整改候选；高级工程师可据此完成最终决策。

> 能力边界：V1.0—V1.2 聚焦“产品设计能否满足注塑制造的几何与规则要求”，不替代模具结构设计，
> 也不等同于模流/CAE 过程仿真。

---

### V2.0：模具结构 DFM（建议 2028 H2 起专项立项）🧰
...
| --- | --- | --- | --- |
| V0.9｜2026.09—12 | 数据契约与全链路追溯 | 多仓库、多版本对象必须准确关联 | Schema 负例、重放、Hash 和跨版本测试 |
| V1.0｜2027 H1 | 常见 B-Rep 特征与基础指标 | 从几何拓扑推断业务语义并稳定计算 | Golden Model、真实产品、工程师标注 |
| V1.1｜2027 H2 | 2D/3D 融合、多因素规则与企业规则 | 多来源事实对应和规则继承冲突 | 图纸标注集、Fusion 审核集、规则发布回归 |
| V1.2｜2028 H1 | 跨特征风险与整改方案权衡 | 多问题关联、复杂几何和经验适用边界 | 复杂产品盲测、方案复核、整改前后对比 |
| V2.0｜2028 H2 起 | 模具结构方案推导 | 一个产品存在多个受约束可行方案 | 模具方案案例库、运动/干涉测试、专家评审 |
| V2.5｜2029 起 | 网格、材料、边界和求解精度 | 输入质量和 CAE 参数共同决定结果可信度 | 基准 Solver、试模和量产数据对比 |
...
| DFM 管理 Web + Django/MySQL | 知识、本体、默认/企业规则、审核、发布、审计和字典共享 | V1.0—V1.2 |
| `dfm-occt-worker` | STEP、B-Rep、产品及模具特征、几何指标、运动/干涉和定位 Artifact | V1.0—V2.0 |
| 二维识别与 Fusion | 图纸 Observation、事实候选和 2D/3D 可审核关系 | V1.1 起 |
...
| V1.1 | 文档智能、2D/3D 几何、知识检索、规则治理 | OCR/视觉工程师、知识平台工程师、规则专家 |
| V1.2 | 复杂几何、跨特征风险、案例检索和工程反馈校准 | 资深 DFM 工程师、几何算法工程师、知识/数据工程师 |
