...
`Agent -> discover (launch 2D sidecar + freeze 3D discovery) -> plan -> start 3D -> report_context (join 2D + 3D)`.
...
2. Call `dfm_project` with `add_input` exactly once for every attached STEP/STP, reserved Parasolid x_t, PDF, PNG, JPG, or JPEG `@file:` reference. When CAD and a drawing are both attached, register both before calling `status`, `discover`, or `plan`; never silently select only the CAD input. When the external OCCT executable is available, STEP intake may return a `viewer_manifest`; Desktop opens it in the 3D viewer. An accepted x_t intake does not mean its geometry reader is available; inspect capability before planning. NX/Parasolid development is deferred and is not the production path for the current milestone.
...
4. Pass `process=injection` or `process=die_casting` when the user has selected it; never infer the process from part shape. Call `dfm_analysis` with `discover`.
   - Drawing vision is launched by `discover` as a non-blocking 2D sidecar using the active main Hermes model, endpoint, and credentials: full-page crop planning, then high-resolution region extraction. In mixed CAD-and-drawing projects, `discover` does **not** wait for this branch: it freezes the available 3D discovery snapshot and returns so the normal 3D `plan`/`start` flow can begin immediately. The two dependent 2D vision calls themselves remain sequential. No separate vision API is configured. Do not call `vision_analyze`, do not open a PDF in a browser, and do not manually call `drawing_context`, `submit_crop_plan`, or `submit_observations` in the normal workflow. An empty crop plan is recorded as a 2D failure instead of silently dropping the drawing.
   - Drawing/geometry fusion is deferred until reporting and must never delay 3D `plan` or `start`. Do not poll or manually rerun the 2D intermediate actions after `discover` returns.
...
5. Inspect the returned DiscoverySnapshot, Feature/Region coverage, provider statuses, `drawing_background`, and open analysis clarifications. A running 2D sidecar is expected and is not a reason to delay 3D planning. Ask only the process adapter's returned missing analysis facts, using the same wait-then-`confirm_fact` rule. Never continue with a stale geometry scope.
...
7. Call `start` only with the analysis plan returned by `plan` (never the discovery plan) and only when the selected capability is `available`. Preserve its `run_id`.
8. `start` is non-blocking. Immediately save the returned `run_id` and pass that exact ID to every subsequent action; never omit it or invent a replacement. For an HTML-capable STEP run (PDF drawing optional), call `report_context` with `wait_seconds=60`; it joins the unchanged 3D run with the background 2D result. If either branch is still running, it returns `ready=false`; call it again only after a meaningful wait. If it returns `status=agent_fusion_required`, call `fusion_context`, use only the returned Observation/Feature/Region IDs, submit defensible links (or an empty array) with `submit_fusion_links`, then call `report_context` again—do not rerun 3D and do not create another plan. The service validates geometry relationships and the Agent never confirms a FusionLink. The desktop gateway also sends a same-conversation report-ready continuation when the background worker reaches `reporting`; treat that notification as a request to continue `report_context`, not as proof that 2D is already complete. The deterministic worker ends at `status=reporting`, `stage=report_editing`, and 98%; this is not success. `render_html` is also non-blocking: it returns `accepted=true` while JobManager renders the report and emits progress/completion notifications. Never say that the pipeline is complete and never call `result` before the run reaches `succeeded` with `report.html` attached. The Agent must never cancel a DFM run; cancellation is reserved for an explicit user-interface action. Runs that do not advertise HTML Runtime support retain the existing background behavior and may complete without HTML.
9. When `report_context` returns `ready=true`, read its inline `runtime` object completely; do not use terminal/read-file to fetch the artifact. When a PDF drawing is present, treat `runtime.drawing_semantics.observations` as the sole drawing-semantic source for reporting; without a drawing the observations array is empty. Do not call `drawing_context` or reinterpret the drawing during this report stage. The **current Hermes conversation model is the final report editor**, not a field copier: synthesize a concise, professional management summary; explain the evaluated risks; prioritize actionable optimization steps; and translate/localize the report prose into the user's language (Chinese for a Chinese request). Preserve part numbers, standard codes, rule IDs, numeric values, operators, units, and backend/version identifiers exactly. Organize only those persisted observations and deterministic Runtime facts into the exact `dfm-html-llm/v1` structure, preserving the complete set of Runtime issue IDs. Faithfully cover every persisted `global_note` value in `part.technical_note`; the Notes may be translated and grouped by topic for readability, but none may be silently omitted or changed in meaning. Clearly qualify engineering implications as analysis when they are not explicit drawing facts, and do not claim that an unevaluated check passed. Call `dfm_analysis` with `action=render_html`, the same `run_id`, and that complete editorial object as `llm_content`. The HTML generator receives this Agent summary plus deterministic Runtime data; it must never infer actual values, thresholds, rule IDs, severity, evidence paths, or new findings from prose. After the action returns `accepted=true`, rely on the background completion notification or use `status` after a meaningful wait; do not rapidly poll. Only this successful action moves the run to `succeeded`/100%, after its background job finishes and validates the HTML. Then call `result`, present the returned `report.html` as the primary human-readable report, and retain JSON/Markdown for traceability. Do not call a second model endpoint and do not bypass this step with mechanically copied Runtime text.
...
Drawing crop planning and Agent interpretation run as a Discovery-launched background sidecar. Drawing-only
objective geometry execution remains unavailable; mixed projects route
objective calculations through the unchanged configured geometry backend while 2D runs independently.
...
- Never submit an Observation without exact semantic crop region evidence, or a
