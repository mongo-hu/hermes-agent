...
`Agent -> visual crop planning -> Agent interpretation -> validated persistence -> plan -> start`.
...
4. Pass `process=injection` or `process=die_casting` when the user has selected it; never infer the process from part shape. Call `dfm_analysis` with `discover`.
   - Drawing vision runs inside `discover` as two background calls using the active main Hermes model, endpoint, and credentials: full-page crop planning, then high-resolution region extraction. In mixed CAD-and-drawing projects, this complete 2D branch runs concurrently with 3D geometry discovery and joins it before fusion; the two dependent vision calls themselves remain sequential. No separate vision API is configured. Do not call `vision_analyze`, do not open a PDF in a browser, and do not manually call `drawing_context`, `submit_crop_plan`, or `submit_observations` in the normal workflow. An empty crop plan is rejected instead of silently dropping the drawing.
...
8. `start` is non-blocking. Immediately save the returned `run_id` and pass that exact ID to every subsequent action; never omit it or invent a replacement. For an HTML-capable STEP run (PDF drawing optional), call `report_context` with `wait_seconds=60` so the same Agent turn can wait efficiently without terminal sleeps or rapid status polling. If it returns `ready=false`, call it again only after a meaningful wait or when the user resumes the run. The desktop gateway also sends a same-conversation report-ready continuation when the background worker reaches `reporting`; treat that notification as a request to complete `report_context` and queue `render_html` immediately, without waiting for another user message. The deterministic worker ends at `status=reporting`, `stage=report_editing`, and 98%; this is not success. `render_html` is also non-blocking: it returns `accepted=true` while JobManager renders the report and emits progress/completion notifications. Never say that the pipeline is complete and never call `result` before the run reaches `succeeded` with `report.html` attached. The Agent must never cancel a DFM run; cancellation is reserved for an explicit user-interface action. Runs that do not advertise HTML Runtime support retain the existing background behavior and may complete without HTML.
...
Drawing crop planning and Agent interpretation run during Discovery. Drawing-only
